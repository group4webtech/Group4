...
cly = nx.circular_layout(g)
my_colors = []
for key, value in cly.items():
...
graph_circle = from_networkx(g, cly, scale=1, center=(0,0))

# !!! Specify colors with node attributes !!!
graph_circle.node_renderer.data_source.data['my_fill_color'] = my_colors
graph_circle.node_renderer.glyph = Circle(size=15,  fill_color='my_fill_color')
...
graph_spring = from_networkx(g, my_points,  scale=1, center=(0,0))

# !!! Specify colors with node attributes !!!

graph_spring.node_renderer.data_source.data['degree'] = list(zip(*g.degree))[1]
graph_spring.node_renderer.data_source.data['my_fill_color'] = my_colors
graph_spring.node_renderer.glyph = Circle(size=15, fill_color="my_fill_color")
...
graph_fd.edge_renderer.glyph = MultiLine(line_color='lightskyblue', line_alpha=1, line_width=3)
...
# Assign the panels to Tabs
tabs = Tabs(tabs=[circle_panel, spring_panel, fd_panel])
